...
class InteroperableTarget:
    ...
    def __init__(self, **kwargs):
        ...
        self.ieee1547_mapping = {k:
            {'mapped_points': v, 'read': self._make_reader(k, v), 'prep_write': self._make_prep_write(k, v)}
            for k, v in self.MAPPING.items()
        }

    def _make_reader(self, ieee1547_name, point_names):
        adjusted_name = self._adjust_name(ieee1547_name)
        def reader():
            if hasattr(self, f'read_{adjusted_name}'):
                callback = getattr(self, adjusted_name)
                return callback()
            else:
                ret_dict = {}
                for point in point_names:
                    ret_dict[ieee1547_name] = self.target_mapping[point]
                return ret_dict
        return reader

    def _make_prep_write(self, ieee1547_name, point_names, prepper_style):
        def one_to_one(values):
            ret_dict = {}
            if not len(point_names) == 1:
                raise ValueError(f'{ieee1547_name} requires one value. Received {len(values)}')
            ret_dict[point_names[0]] = values

        def map_list(values):
            ret_dict = {}
            if not isinstance(values, list) and len(point_names) == len(values):
                raise ValueError(f'{ieee1547_name} requires {len(point_names)} values. Received {len(values)}')
        ...
        adjusted_name = self._adjust_name(ieee1547_name)
        if prepper_style == 'custom' and hasattr(self, f'prep_write_{adjusted_name}'):
            return getattr(self, adjusted_name)
        elif prepper_style == 'map_list':
            return map_list
        elif prepper_style == 'one_to_one':
            return one_to_one
        elif prepper_style == 'to_each':
            return to_each

    @staticmethod
    def _adjust_name(ieee1547_name: str):
    ...
    def factory(cls, config):
        ...
        if not target_class_name and target_type in cls.KNOWN_TARGET_TYPES:
            target_class = cls.KNOWN_TARGET_TYPES[target_type]
